...
text-align: left;
    ...
    <body>
        <form action="#" method="POST">
            <br>
            <div class="header">
            <h2 align="center"> Въведете фак. номер на студента и наименованието на дисциплината, на които искате да промените оценкaтa:</h2>
            </div>
            <br>
            <br>
...
if (isset($_POST['submit'])) {
    ...
    while($row = mysqli_fetch_assoc($resST)) {
       ...
       if ($_POST['Fac_num'] == $row['Facility_num']) { $flagST=true; $_SESSION['Fac_num']=$_POST['Fac_num']; }
    }
    
    if (!$flagST) {
        die("<ul><li>Студент с такъв факултетен номер не съществува</li></ul> !");
    }
    ...
    while($row = mysqli_fetch_assoc($resDS)) {
       ...
       if ($_POST['DS_Name'] == $row['DS_Name']) { $flagDS=true; $_SESSION['Discipline']=$row['DS_Id']; }
    }
    ...
    $res= mysqli_query($dbConn,"SELECT * FROM Rating JOIN Student ON ( Facility_num = ST_FK_R) JOIN Discipline ON ( DS_ID = DS_FK_R) ");
          ...
          while ($row= mysqli_fetch_assoc($res)) { if ($_POST['Fac_num'] == $row['ST_FK_R'] && $_SESSION['Discipline'] != $row['DS_FK_R']) $flagSTDS=false; }
   ...
   {
        header('Location: http://localhost/NewPhpProject2/update2_rating.php');
    }
